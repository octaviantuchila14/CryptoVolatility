...
RSpec.describe Market, type: :model do
  ...
  it "evaluates the beta of a currency" do
    3.times do |i|
      er = FactoryGirl.create(:exchange_rate, last: 10, time: Time.now - i*days)
  ...
  it "evaluates the beta of a currency" do
    ...
    3.times do |i|
      erm = FactoryGirl.create(:exchange_rate, last: val[0][i], time: Time.now - (3 - i)*days, subject: @market.name)
      ercr = FactoryGirl.create(:exchange_rate, last: val[1][i], time: Time.now - (3 - i)*days, subject: @currency.name)
      @market.exchange_rates << erm
      @currency.exchange_rates << ercr
    end
    @market.exchange_rates << FactoryGirl.create(:exchange_rate, last: val[0][3], time: Time.now)
